...
@section('main')
    ...
    <form class="form-content h-adr" action="{{ route('profile.update') }}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="image-group">
            <img class="image-group__icon" src="{{ asset('storage/' . $user->img_url) }}" alt="ユーザーアイコン">
            <label class="image-group__label">
                <input class="image-group__input" type="file" name="file" id="image" onchange="previewFile()">画像を選択する
            </label>
            <script>
        ...
        <label class="form-content__label">郵便番号
            <input class="form-content__input p-postal-code" type="text" size="8" maxlenght="8" name="postcode" value="{{ $user->postcode }}">
        </label>
        @error('postcode')
            <div class="form-content__error">{{ $message }}</div>
        @enderror
        ...
        <label class="form-content__label">住所
            <input class="form-content__input p-region p-locality p-street-address" type="text" name="address" value="{{ $user->address }}">
        </label>
        @error('')
            <div class="form-content__error">{{ $message }}</div>
        @enderror
        ...
        <label class="form-content__label">建物名
            <input class="form-content__input" type="text" name="building" value="{{ $user->building }}">
        </label>
